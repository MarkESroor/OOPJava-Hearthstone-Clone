...
public abstract class Hero implements MinionListener {
	...
	public void onLivingMinionUpdated(Minion minion) {
		int tmp = getField().indexOf(minion);
		listener.onLivingMinionChange(field,tmp);
	}
	...
	public void castSpell(MinionTargetSpell s, Minion m)
		...
		validator.validateManaCost((Spell) s);
		Card x= (Card) s;
		if((x.getName()=="Polymorph" && (m.getName()=="Prophet Velen"))) {
			listener.specialMinion(m.getName(),0,this);}
		s.performAction(m);
		castSpellCommons((Spell) s);
	...
	public Card drawCard() throws FullHandException, CloneNotSupportedException {
		if (fatigueDamage > 0) {
			...
			listener.OnFatigue(this,fatigueDamage);
			fatigueDamage++;
		...
		if(this!=null && listener!=null && deck.size()==0)
			listener.OnFatigue(this,0);
		try {
		
		if (hand.size() == 10)
			throw new FullHandException("My hand is too full !!!", c);
		hand.add(c);
		listener.onCardDrawn(c,this);
		if (fieldContains("Chromaggus") && hand.size() < 10) {
			hand.add(c.clone());
			listener.onCardDrawn(c,this);
			//listener.OnUpdateCard(this, c);
		}
		
		
		}
		catch(FullHandException e1) {
			listener.burnCard(c,this);
		}
		return c;
	...
	public void onMinionDeath(Minion m) {
		if(m.getName()=="Prophet Velen" )
			listener.specialMinion(m.getName(),0,this);
		int i = field.indexOf(m);
		listener.onFieldUpated(field,i);
		field.remove(m);
